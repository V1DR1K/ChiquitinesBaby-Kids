...
const HIGHLIGHTS = [
  { title: 'Telas Orgánicas', subtitle: 'Cuidado natural en cada hilado', description: 'Prendas de algodón orgánico certificado, especiales para prematuros desde los 700 g y bebés.', action: 'Ver ropa de diseño', tab: 'clothing' as const, categories: ['clothing'], image: CLOTHES_IMG },
  { title: 'Vestimenta urbana', subtitle: 'Comodidad para todos los días', description: 'Telas prelavadas, resistentes y duraderas, con calce cómodo y moderno para niños de 12 meses a 8 años.', action: 'Ver ropa urbana', tab: 'clothing' as const, categories: ['clothing'], image: CLOTHES_IMG },
  { title: 'Calzados', subtitle: 'Del talle 14 al 26', description: 'Modelos antideslizantes para no caminantes del 14 al 17 y anatómicos para caminantes del 18 al 26.', action: 'Explorar calzado', tab: 'footwear' as const, categories: ['footwear'], image: FOOTWEAR_AND_BAG_IMG },
  { title: 'Bolsos, mochilas y blanquería', subtitle: 'Todo para acompañar al bebé', description: 'Bolsos y mochilas funcionales, sábanas, cambiadores, toallones y accesorios para tener todo a mano.', action: 'Explorar bolsos y accesorios', tab: 'accessories' as const, categories: ['linen', 'bags'], image: FOOTWEAR_AND_BAG_IMG },
];
...
export default function CatalogSection({ onSelectProduct }: { onSelectProduct: (product: ProductCatalogItem) => void }) {
  ...
  const filteredHighlights = activeTab === 'all' || activeTab === 'accessories'
    ? activeTab === 'all' ? HIGHLIGHTS : HIGHLIGHTS.filter((highlight) => highlight.tab === 'accessories')
    : HIGHLIGHTS.filter((highlight) => highlight.categories.includes(activeTab));
  const tabs = [
